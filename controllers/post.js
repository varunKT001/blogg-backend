...
async function getBlog(req, res) {
  ...
  try {
    let result = await pool.query(`SELECT * FROM blogs WHERE blogid = $1`, [
      blogid,
    ]);
    let blogs = result.rows;
    return res.json({
...
async function getUserBlogs(req, res) {
  ...
  try {
    let result = await pool.query(
      `SELECT * FROM blogs WHERE author = $1 ORDER BY blogid DESC`,
      [req.params.username]
    );
    let blogs = result.rows;
    return res.json({
...
async function postBlog(req, res) {
  ...
  try {
    let result = await pool.query(
      `INSERT INTO blogs (tittle, author, date, edited, content, contentraw, description, likes) VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`,
      [
        blog.tittle,
        blog.author,
        blog.date,
...
async function deleteUserBlog(req, res) {
  ...
  blogid = req.params.blogid;
  try {
    let result = await pool.query(`DELETE FROM blogs WHERE blogid = $1`, [
      blogid,
    ]);
    let result_1 = await pool.query(`DELETE FROM likes WHERE blogid = $1`, [
      blogid,
    ]);
    return res.json({
    ...
    if (result.rows.length == 0) {
      try {
        let result_1 = await pool.query(
          `INSERT INTO likes (userid, blogid) VALUES ($1, $2)`,
          [userid, blogid]
        );
        let updateLikes = await pool.query(
          `UPDATE blogs SET likes = likes + 1 WHERE blogid = $1`,
          [blogid]
        );
        return res.json({
          message: "liked",
...
async function likedOrNot(req, res) {
  ...
  console.log("likedOrNot request recieved", userid);
  try {
    let result = await pool.query(
